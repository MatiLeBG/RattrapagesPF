  ...
  struct
    ...
    let rec combinaisons k l = 
      ND.(
        ...
        | 0,_ -> return []
        | _,[] -> zero
        | _,t::q -> map (fun l -> t::l) (combinaisons (k-1) q) ++ combinaisons k q
      )
  end
  ...
  struct
    ...
    let rec insertion e l =
      ND.(
        match l with 
        | [] -> return [e]
        | t::q -> return (e::l) ++ map (fun x -> t::x) (insertion e q)
      )
        ...
        let rec aux n i =
          if n = i then
            ...
            if n < i then 
              ...
              map (fun x -> i::x) (aux (n-i) i) ++ (aux n (i+1))
        in
          aux n 1 
      )
...
module NDIter : MONADE_PLUS_PRINT =
  struct
    ...
    let return =  fun e ->  
      Tick (lazy (Some (e, zero)))

    let rec map = fun f flux ->
      Tick (lazy (
          match uncons flux with 
          | None -> None
...
module NDRandom : MONADE_PLUS_PRINT =
  struct
    ...
    (* à compléter *)
    let ( ++ ) = fun a b -> if Random.bool() then a else b
    let zero = fun () -> None
