...
and parse_I : (char, inst) parser = fun flux ->
  (
    (return []) ++ (parse_C >>=  fun c -> p_ptvirg >>= fun _ -> parse_I >>= fun i -> return (c::i))
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
  (
    p_repeat >>= fun _ -> p_entier >>= fun n -> parse_P >>= fun p -> return (Repeat(n,p)) 
    ++  (p_move >>= fun _ -> p_entier >>= fun n -> return (Move(n)))
    ++ (p_turn >>= fun _ -> p_entier >>= fun n -> return(Turn(n)))
...
let prog_1 = [On; Repeat(360, [Move(1); Turn(1)]); Off]
let run_logo prog =
  begin
    Graphics.open_graph " 1920*1080";
...
let exo2 filename =  
  let sol = parse_logo (flux_of_file filename) in
    match Solution.uncons sol with
    | None -> failwith "Probleme"
    | Some(l, _) -> run_logo l; ()
