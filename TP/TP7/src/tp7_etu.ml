...
let rec unless flux cond f =
    Tick(lazy( match (Flux.uncons flux) with
    | None -> None
    | Some(t,q) ->  if cond t then 
                      Flux.uncons(f t)
                    else 
                      Some(t, (unless q cond f))
    ))
...
struct
  let contact_x x dx =
    let(xmin, xmax) = F.box_x in 
      (x <= xmin && dx < 0. ) || (x >= xmax && dx > 0.)  

    let contact_y y dy =
      let(ymin, ymax) = F.box_y in 
        (y <= ymin && dy < 0. ) || (y >= ymax && dy > 0.) 
    ...
    let rebond : etat -> (float * float) =
      fun ((x,y), (dx,dy)) -> 
        (rebond_x x dx, rebond_y y dy)
    ...
    let run_ff : etat -> etat Flux.t = (*assert false*) (* A COMPLETER *)
    fun ((x0,y0),(vx0,vy0)) ->
      let acc = Flux.constant g in
      let vit = Flux.map (fun (vx,vy) -> (vx +. vx0,vy +. vy0)) (integre F.dt acc) in
      let pos = Flux.map (fun (x,y) -> (x +. x0, y +. y0)) (integre F.dt vit) in
    ...
    let rec run e =
      unless (run_ff e) contact (fun ((x,y),(dx,dy)) -> run ((x,y),(rebond ((x,y),(dx,dy)))))
      
  

end 

let etat0 = ((320., 240.), (10.,50.))

module FF = Bouncing(F)

module  OurDraw =  Drawing (F)

let main () = OurDraw.draw (FF.run etat0)